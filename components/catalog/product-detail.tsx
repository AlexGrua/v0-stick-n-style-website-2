    ...
    <div className={cn("grid gap-6 lg:grid-cols-[1.2fr_1fr]", className)}>
      ...
      <div>
        <div className="relative aspect-square overflow-hidden rounded border">
          <Image src={activeImage || "/placeholder.svg"} alt={`${product.name} main`} fill className="object-cover" />
          <button
            onClick={prev}
              ...
              <Image
                src={
                  i === 0 && ((COLORS[colorIdx]?.image as string | undefined) || product.thumbnailUrl)
                    ...
                    : `/placeholder.svg?height=160&width=160&query=${encodeURIComponent(`${product.name} thumb ${i + 1}`)}`
                }
                alt={`thumb ${i + 1}`}
                fill
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="grid grid-cols-5 gap-2">
            {COLORS.map((c, i) => (
              <button
                ...
                className={cn("h-10 w-full rounded border", i === colorIdx && "ring-2 ring-emerald-600")}
                aria-label={c.name}
                title={c.name}
          ...
          <div className="mt-2 text-xs text-muted-foreground">Selected: {COLORS[colorIdx]?.name}</div>
        </div>
        ...
        {/* Specs */}
        <div className="flex flex-wrap gap-2">
          <Badge variant="secondary">Pcs/Box: {product.pcsPerBox}</Badge>
          <Badge variant="secondary">Box KG: {product.boxKg}</Badge>
          <Badge variant="secondary">Box m³: {product.boxM3}</Badge>
        </div>
